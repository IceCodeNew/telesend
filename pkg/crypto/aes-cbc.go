...
func EncryptWithAESCBC(asciiIV, asciiKey []byte, msg string) (string, error) {
	...
	// _block_size_minus_one is 1111b
	_block_size_minus_one, lenMsg := aes.BlockSize-1, len(msg)
	// make the length of msg a multiple of aes.BlockSize
	// only works when all binary digits of _block_size_minus_one is 1
	_cap := (lenMsg + _block_size_minus_one) &^ _block_size_minus_one
